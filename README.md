...
### The web app (collect, re-analyse and read a run from the browser)
    ...
    python -m uvicorn src.server:app --port 8800
...
### The offline page (what we present from)
    ...
    python -m src.site --run-id run_2026...

Writes `web/site.html` with the run and its fonts embedded. Open the file on any
